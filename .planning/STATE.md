...
- **🚨 URGENT: Phase 12.2 re-host layer needed** — Phase 12.1 Azure Front Door FAILED: Meta Graph API `image_url` fetcher rejects AFD hostnames (default `*.z01.azurefd.net` AND custom domain `cdn.propulsar.ai` — both verified with code 9004/2207052 on execs 14273 and 14938 respectively). Control test with Ideogram direct URL from same dev IP succeeded (200 + container id) → confirms Meta block is AFD-specific, not IP-specific. Options D/B/E band-aid back in production (rollback deployed 2026-04-23 night, main versionId 966dc454). Original plan in 12.1-HANDOFF.md was Cloudflare R2 — **superseded 2026-07-31** by a smoke test proving Meta also accepts the existing Hostinger VPS/EasyPanel wildcard hostname (`*.bacu5y.easypanel.host`), with zero new accounts needed (reuses `hostinger-api-key` in Key Vault + `EASYPANEL_API_TOKEN` already in `propulsar-atiende/.env`). See `.planning/phases/12.1-.../12.2-HOSTINGER-SMOKE-TEST.md` for full evidence + updated recommendation. Next: build a real re-host service on that VPS (not Cloudflare R2) — expected ~3-4h next session, similar shape to the old R2 plan's Wave 2-4.
...
Next recommended: Start fresh session with `/gsd:insert-phase 12.2` for a Hostinger VPS re-host layer (NOT Cloudflare R2 — see 2026-07-31 smoke test superseding note above) — plan will reference exec-captures evidence + `12.2-HOSTINGER-SMOKE-TEST.md` to unblock Phase 13. No new prerequisites — `hostinger-api-key` (Key Vault) and `EASYPANEL_API_TOKEN` (`propulsar-atiende/.env`) already validated working.
